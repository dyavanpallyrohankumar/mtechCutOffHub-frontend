        ...
        <div className="hidden md:flex items-center gap-1">
          {navLinks.map((link) => (
            <Link
              key={link.path}
              to={link.path}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${location.pathname.startsWith(link.path)
        ...
        <div className="md:hidden border-t border-border/50 bg-card/95 backdrop-blur-xl">
          {navLinks.map((link) => (
            <Link
              key={link.path}
              to={link.path}
              className={`block px-6 py-3 text-sm font-medium transition-colors ${location.pathname.startsWith(link.path)
